...
<header class="navbar">
  <a href="/index.php" class="logo">
    <?php if (!empty($config['logo'])): ?>
      <img src="/<?= e($config['logo']) ?>" alt="logo" onerror="this.style.display='none'">
    <?php endif; ?>
    <span><?= e($config['nome_site'] ?? 'Reuse Chic') ?></span>
  </a>
  <form class="search" action="/catalogo.php" method="get">
    <input type="text" name="q" placeholder="Digite aqui o que deseja buscar">
    <button type="submit">🔍</button>
  </form>
  <nav class="nav-links">
    <?php if (isLoggedCliente()): ?>
      <a href="/perfil.php">👤 <?= e($_SESSION['usuario_nome']) ?></a>
      <a href="/logout.php">Sair</a>
    <?php else: ?>
      <a href="/inicial.php">👤 Entrar</a>
    <?php endif; ?>
